...
struct SPGrid_Geometry<3> {
    ...
    SPGrid_Geometry(const ucoord_t xsize_input, const ucoord_t ysize_input, const ucoord_t zsize_input,
        ...
        : xsize(xsize_input), ysize(ysize_input), zsize(zsize_input), block_xsize(1u << block_xbits), block_ysize(1u << block_ybits), block_zsize(1u << block_zbits), zsize_padded(Next_Power_Of_Two(std::max({ xsize, ysize, zsize, block_zsize }))), ysize_padded(std::max({ Next_Power_Of_Two(std::max(ysize, xsize)), zsize_padded >> 1, block_ysize })), xsize_padded(std::max({ Next_Power_Of_Two(xsize), zsize_padded >> 1, block_xsize }))
    {
    }
...
struct SPGrid_Geometry<2> {
    ...
    SPGrid_Geometry(const ucoord_t xsize_input, const ucoord_t ysize_input, const int block_xbits, const int block_ybits)
        : xsize(xsize_input), ysize(ysize_input), block_xsize(1u << block_xbits), block_ysize(1u << block_ybits), ysize_padded(Next_Power_Of_Two(std::max({ xsize, ysize, block_ysize }))), xsize_padded(std::max({ Next_Power_Of_Two(xsize), ysize_padded >> 1, block_xsize }))
    {
    }
